...
const exportDefaultExportNameMap_2: SuseeTypes.NamesSets = [];
const exportDefaultImportNameMap_2: SuseeTypes.NamesSets = [];
...
function resetExportDefaultState() {
	exportDefaultExportNameMap_2.length = 0;
	exportDefaultImportNameMap_2.length = 0;
			...
			function visitor(node: ts.Node): ts.Node {
				if (ts.isCallExpression(node)) {
					if (ts.isIdentifier(node.expression)) {
						...
						const mapping = exportDefaultImportNameMap_2.find(
...
function exportDefaultExportHandler(compilerOptions: ts.CompilerOptions) {
	return ({ fileName, sourceCode, sourceFile }: SuseeTypes.DepFileObject) => {
		const transformer: ts.TransformerFactory<ts.SourceFile> = (context) => {
			const { factory } = context;
			function visitor(node: ts.Node): ts.Node {
				...
				if (
					...
					if (exp && def) {
						const base = genName.getName(prefixKey, identifierName);
						exportDefaultExportNameMap_2.push({
							base,
							file: filename,
							newName: base,
							isEd: true,
...
function exportDefaultImportHandler(compilerOptions: ts.CompilerOptions) {
	return ({ fileName, sourceCode, sourceFile }: SuseeTypes.DepFileObject) => {
		const transformer: ts.TransformerFactory<ts.SourceFile> = (context) => {
			const { factory } = context;
			function visitor(node: ts.Node): ts.Node {
				if (ts.isImportDeclaration(node)) {
					...
					if (
						...
						const mapping = exportDefaultExportNameMap_2.find(
							(v) => v.file === _name,
						);
						if (mapping) {
							exportDefaultImportNameMap_2.push({
								base,
								file: fileName,
...
const exportDefaultHandlers = async (
...
export default exportDefaultHandlers;
