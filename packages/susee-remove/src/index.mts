...
const typeObj: SuseeTypes.TypeObj = {};
...
const esmExportRemoveHandler = (compilerOptions: ts.CompilerOptions) => {
		...
		const transformer: ts.TransformerFactory<ts.SourceFile> = (context) => {
			...
			const visitor = (node: ts.Node): ts.Node => {
				...
				if (
					...
					if (utils.check.moduleType(sourceFile, fileName).isCommonJs) {
...
const removeHandlers = async (
	...
	const resolved = resolves([
		[importAllRemoveHandler, compilerOptions, removedStatements],
