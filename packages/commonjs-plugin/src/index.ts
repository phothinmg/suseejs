...
function commonjsImportsHandler(
	...
	return (dep: DepsFile) => {
		if (
			dep.moduleType === "cjs" &&
			(dep.fileExt === ".js" || dep.fileExt === ".cjs")
		) {
			...
			for (const stmt of sourceFile.statements) {
				properties = [...properties, ...utils.gen.findProperty(stmt)];
			}
			...
			let _content = utils.gen.transformFunction(
				transformer,
				sourceFile,
				compilerOptions,
			);
...
function suseeCommonJS(): SuseePlugin {
	return {
		type: "dependency",
		async: true,
		name: "@suseejs/commonjs-plugin",
		func: async (deps, compilerOptions) => {
			const resolvedHandler = utils.promises.resolve([
